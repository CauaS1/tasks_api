...
export const saveTask = async (request: Request, response: Response) => {
  const task = await getRepository(Tasks).save(request.body); // vai vir todos os dados da task, tipo o do body do insominia
...
export const finishedTask = async (request: Request, response: Response) => {
  ...
  if(task.affected == 1) {
    const taskUpdated = await getRepository(Tasks).findOne(id);
    return response.json({ messge: "Task was finished! "}); //vai passar uma mensages 
  }
...
export const removeTask = async (request: Request, response: Response) => {
  ...
  if(task.affected == 1) {
    const taskUpdated = await getRepository(Tasks).findOne(id);
    return response.json({ messge: "Task was removed! "}); //vai passar uma mensages 
  }
